...
// CATÁLOGO MAYORISTA — Mormaii Sports Verão 27 (vista previa).
// Grilla producto×color, filtros, modal con galería, carrito por curvas
// pre-pack y checkout para locales. Datos: /catalog-data.json + fotos
// reales en /catalog/. No toca /s/[token].
...
export default function CatalogoMayorista() {
  ...
  const totalUsd = cartLines.reduce(
    (s, l) => s + l.p.mayorista * CURVA_UNITS * l.qty,
    0,
  );
  ...
  if (step === "checkout")
    return <Checkout lines={cartLines} totalCurvas={totalCurvas} totalUsd={totalUsd}
  ...
  return (
    <main className="min-h-screen bg-white text-stone-900 pb-28">
      <div className="bg-amber-50 border-b border-amber-200 px-5 py-2 text-center text-[12px] text-amber-800">
        Vista previa · catálogo mayorista · datos y fotos reales · no toca la encuesta en vivo
      </div>
...
function Checkout({ lines, totalCurvas, totalUsd, onBack, onDone }: {
  lines: CartLine[]; totalCurvas: number; totalUsd: number; onBack: () => void; onDone: () => void;
}) {
  ...
  const submit = () => {
    ...
    try {
      const all = JSON.parse(localStorage.getItem("kemsa_pedidos") || "[]");
      all.push({
        empresa, contacto, phone, comment, totalCurvas, totalUsd,
        created_at: new Date().toISOString(),
        items: lines.map((l) => ({
          code: l.p.code, name: l.p.name, color: l.v.colorName,
          curvas: l.qty, unidades: l.qty * CURVA_UNITS,
        })),
      });
      localStorage.setItem("kemsa_pedidos", JSON.stringify(all));
    } catch {}
    onDone();
  };
  return (
    <main className="min-h-screen bg-white p-5 max-w-lg mx-auto">
      ...
      <button onClick={submit} className="mt-6 w-full bg-black text-white font-medium rounded-full py-3.5">Enviar pedido</button>
      <style jsx global>{`
